...
//		ArrayList<Answer> answers = dao.getAnswersForStudent(student.getId());
//		session.setAttribute("studentAnswers", answers);
		HttpSession session = request.getSession();
		User student = (User) session.getAttribute("user");
		...
		try {
			...
			response.sendRedirect(request.getContextPath() + "/student/certification.jsp");
